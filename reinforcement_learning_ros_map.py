...
class Maze:
    ...
    def show_maze(self):
        ...
        ax.scatter(self.start_position[0], self.start_position[1], color='red', label='Starting Position', s=10) 
        ax.scatter(self.goal_position[0], self.goal_position[1], color='cyan', label='Goal Position', s=10)  
        # Add legend
        ax.legend()
...
class QLearningAgent:
    def __init__(self, maze, learning_rate=0.1, discount_factor=0.9, exploration_start=1.0, exploration_end=0.01, num_episodes=100):
        ...
        self.q_table = np.zeros((maze.maze_height, maze.maze_width, 8)) # 4 actions: Up, Down, Left, Right
...
def test_agent(agent, maze, actions, num_episodes=1):
    ...
    # Plot the start and goal positions
    ax.scatter(maze.start_position[0], maze.start_position[1], color='red', label='Starting Position', s=10) 
    ax.scatter(maze.goal_position[0], maze.goal_position[1], color='cyan', label='Goal Position', s=10)  
...
def main():
    ...
    start_grid_coor = (100,100)
    end_grid_coor = (15,11)
    planned_path = None
    num_of_ep = 1000

    with open('UoM_Intern_2024\maze.txt', 'r') as file:
        content = file.read()
        array_list = ast.literal_eval(content)
    maze_layout = np.array(array_list)
